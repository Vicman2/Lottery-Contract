...
describe("Lottery contract", () => {
    ...
    it("allows multiple accounst to enter", async () => {
        await lottery.methods.enter().send({
            from:accounts[0], 
            value: web3.utils.toWei("0.02", "ether")
        })
    ...
    it("Requres a minimum amount of ether to enter", async() =>{
        try {
            await lottery.methods.enter().send({
                from:accounts[0], 
                value:0
    ...
    it("Only manager should call pick winner", async () =>{
        try {
    ...
    it("Sends money to the winner and resets the players", async () =>{
        ...
        assert(difference > parseInt(web3.utils.toWei("1.8", "ether")));
    })
